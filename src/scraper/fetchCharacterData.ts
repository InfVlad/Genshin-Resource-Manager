...
import { characterDataSchema, zodCharErrorLogger } from './validation';
...
export async function getCharacterData(characterUrl: string): Promise<CharacterData> {
  try {
    ...
    const parseResult = characterDataSchema.safeParse(character);
    return zodCharErrorLogger(parseResult, character.name);
  } catch (error) {
